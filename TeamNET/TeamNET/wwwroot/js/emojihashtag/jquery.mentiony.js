...
(function ($) {
    ...
    jQuery.fn.mentiony = function (method, options) {
        var defaults = {
            ...
            templates:          {
                ...
                listItem:         '<li class="mentiony-item" data-item-id="" style="">' +
                                  '<div class="row">' +
                                  ...
                                  '<p class="title font-weight-bold">Company name</p>' +
                                  //'<p class="help-block">Addition information</p>' +
                                  '</div>' +
                                  '</div>' +
                                  '</li>',
                normalText:       '<span class="normal-text">&nbsp</span>',
                highlight:        '<span></span>',
                highlightContent: '<span data-item-id="[ITEM_ID]" class="mentiony-link font-weight-bold" style="color: [COLOR]">[TEXT]</span>',
            }
        };
    ...
    var MentionsInput = function (settings) {
        var elmInputBoxContainer, elmInputBoxContent, elmInputBox,
            ...
            elmInputBoxContentAbsPosition = {top: -220, left: 0},
        ...
        function initTextArea(ele) {
            ...
            elmInputBoxInitialWidth = settings.inputWidth ?? elmInputBox.prop('scrollWidth');
            elmInputBoxInitialHeight = settings.inputHeight ?? elmInputBox.prop('scrollHeight');

            // Container
            elmInputBoxContainer = settings.app ?? $(settings.templates.container.replace('[ID]', inputId));
            elmInputBoxContent = settings.editor ?? $(settings.templates.content.replace('[ID]', inputId));
        ...
        function populateDropdown(keyword, responseData) {
            ...
            if (responseData.length) {
                ...
                responseData.forEach(function (item, index) {
                    ...
                    listItem.attr('style','color:' + item.color);
        ...
        function showDropDown() {
            ...
            popoverEle.css({
                display: 'block',
                top:     curAbsPos.y - (elmInputBoxContentAbsPosition.top - $(document).scrollTop()) + (editableContentLineHeightPx -220),
                left:    curAbsPos.x - elmInputBoxContentAbsPosition.left -10
            });
        }
        ...
        function choseMentionOptions(chooseByMouse) {
            ...
            var highlightContentNode = $(settings.templates.highlightContent
                ...
                .replace('[COLOR]', currentMentionItemData.color)
            );
